...
class Timemanagement_Model_MyTimesheet extends Zend_Db_Table_Abstract
{
	...
	public function getMonthTimesheetData($empId,$year,$month) { 
	...
	public function getWeeklyTimesheetData($empId,$year,$month,$week,$flag='') { 
		
		$where = '';
		if($flag != 'view'){
			$where .= " and  p.project_status NOT IN ('draft','hold','completed')";
		}
		$select = $this->select()
				  ->setIntegrityCheck(false)
...
//	public function getWeekDaysStatus($empId,$year,$calWeek){
	...
	public function updateDayStatus($empId,$year,$month,$week){
		...
		$select = $this->select()
				->setIntegrityCheck(false)
				->from(array('s' => 'tm_ts_status'),array(
				 'sun_status' => 'IF(FIND_IN_SET("saved",GROUP_CONCAT(DISTINCT s.sun_project_status))!=0,"saved",
    			IF(FIND_IN_SET("submitted",GROUP_CONCAT(DISTINCT s.sun_project_status))!=0,"submitted","no_entry"))',
				 'mon_status' => 'IF(FIND_IN_SET("saved",GROUP_CONCAT(DISTINCT s.mon_project_status))!=0,"saved",
			    IF(FIND_IN_SET("submitted",GROUP_CONCAT(DISTINCT s.mon_project_status))!=0,"submitted", "no_entry"))',
				 'tue_status' => 'IF(FIND_IN_SET("saved",GROUP_CONCAT(DISTINCT s.tue_project_status))!=0,"saved",
			    IF(FIND_IN_SET("submitted",GROUP_CONCAT(DISTINCT s.tue_project_status))!=0,"submitted","no_entry"))',
				 'wed_status' => 'IF(FIND_IN_SET("saved",GROUP_CONCAT(DISTINCT s.wed_project_status))!=0,"saved",
			    IF(FIND_IN_SET("submitted",GROUP_CONCAT(DISTINCT s.wed_project_status))!=0,"submitted","no_entry"))',
				 'thu_status' => 'IF(FIND_IN_SET("saved",GROUP_CONCAT(DISTINCT s.thu_project_status))!=0,"saved",
			    IF(FIND_IN_SET("submitted",GROUP_CONCAT(DISTINCT s.thu_project_status))!=0,"submitted","no_entry"))',
				 'fri_status' => 'IF(FIND_IN_SET("saved",GROUP_CONCAT(DISTINCT s.fri_project_status))!=0,"saved",
			    IF(FIND_IN_SET("submitted",GROUP_CONCAT(DISTINCT s.fri_project_status))!=0,"submitted","no_entry"))',
				 'sat_status' => 'IF(FIND_IN_SET("saved",GROUP_CONCAT(DISTINCT s.sat_project_status))!=0,"saved", 
			    IF(FIND_IN_SET("submitted",GROUP_CONCAT(DISTINCT s.sat_project_status))!=0,"submitted","no_entry"))' 
				))
	...
	public function updateWeekStatus($empId,$year,$month,$week){
		...
		IF(FIND_IN_SET('saved',CONCAT(sun_status,',',mon_status,',',tue_status,',',wed_status,',',thu_status,',',fri_status,',',sat_status))!=0,'saved', 'no_entry'))"))
