...
class Default_Form_addemployeeoncalls extends Zend_Form
{
	public function init()
	{
		$this->setMethod('post');
		$this->setAttrib('id', 'formid');
		$this->setAttrib('name', 'addemponcalls');
		...
		$emp_oncall_limit->setLabel("Allot Oncall Limit");
		...
		$emp_oncall_limit->addValidator("regex",true,array(
                           ...
                           'messages'=>array(
                               'regexNotMatch'=>'Oncall limit must be in the range of 0 to 100.'
                           )
        	));	
		...
		$submitbutton->setAttrib('onclick', 'validateEmployeOncalls()');
		$submitbutton->setLabel('Save');

		$this->addElements(array($id,$user_id,$businessunit_id,$department_id,$emp_oncall_limit,$alloted_year,$submit,$submitbutton));
