...
class Default_RejectedoncallsController extends Zend_Controller_Action
{
			  ...
			  $user_id = $useridArr[0]['user_id'];
				if($user_id == $loginUserId)
				{
					...
					if(count($elements)>0)
					{
						...
						$data = $data[0];
						if(!empty($data) && $data['oncallstatus'] == 'Rejected')
							{
							...
							if($employeeoncalltypeArr != 'norows')
								...
								$oncallrequestform->comments->setValue($data['approver_comments']);
