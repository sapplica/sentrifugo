...
class Services_IndexController extends Zend_Rest_Controller
{
    ...
    */
	...
	public function viewoncalldetails($params_arr)
        {
            $result = array();
			$status = 0;
			$message = "No data found.";
            ...
            if(isset($params_arr['role_id']) && $params_arr['role_id'] != '' && isset($params_arr['group_id']) && $params_arr['group_id'] != '')
            {
				...
				if(isset($params_arr['oncallflag']))
				{
					if($params_arr['oncallflag'] == 'pending')
					   $status_flag = 1;
					else if($params_arr['oncallflag'] == 'rejected')
					   $status_flag = 3;
					else if($params_arr['oncallflag'] == 'cancel')
					   $status_flag = 4;
					else if($params_arr['oncallflag'] == 'approved')
					   $status_flag = 2;
				}

                $privilege_flag = sapp_Global::_check_menu_access(ONCALLS,$group_id,$role_id);
                if($privilege_flag == 'Yes')
                {
					if(isset($params_arr['userid']) && $params_arr['userid'] !='')
					{
						$oncallsmodel = new Services_Model_Oncalls();
						$total_data = $oncallsmodel->getOncalldata($page_no,$per_page,$params_arr['userid'],$status_flag);
						if(!empty($total_data['rows']) && count($total_data['rows']) > 0)
						{
						    $result = $total_data['rows'];
							$role = $total_data['role'];
					...
					if($status_flag == 1)
					{
						$add_flag = sapp_Global::_checkprivileges(ONCALLREQUEST,$group_id,$role_id,"add");
						$edit_flag = sapp_Global::_checkprivileges(PENDINGONCALLS,$group_id,$role_id,"delete");
					}
					else
					{
						$add_flag = sapp_Global::_checkprivileges(ONCALLREQUEST,$group_id,$role_id,"add");
		...
		public function oncallrequest($params_arr)
        {
            $result = array();
			$status = 0;
			$message = "No data found.";
            ...
            if(isset($params_arr['role_id']) && $params_arr['role_id'] != '' && isset($params_arr['group_id']) && $params_arr['group_id'] != '')
            {
                ...
                if($privilege_flag == 'Yes')
                {
					...
					{
						$oncallsmodel = new Services_Model_Oncalls();
						$employee_data = $oncallsmodel->getempdetails($params_arr['userid']);
						//echo "<pre>";print_r($employee_data);exit;
						if(!empty($employee_data) && count($employee_data) > 0)
						{
						    if(empty($employee_data['repmanagerdetails']) || empty($employee_data['availableoncalls']) || empty($employee_data['oncalltypes']))
							{
								if(empty($employee_data['repmanagerdetails']))
									$messagearray['repmanager'] = 'Reporting manager is not assigned yet. Please contact your HR.';
		...
		public function saveoncallrequest($params_arr)
        {
            $result = array();
			$status = 0;
			$message = "No data found.";
            ...
            if(isset($params_arr['role_id']) && $params_arr['role_id'] != '' && isset($params_arr['group_id']) && $params_arr['group_id'] != '')
            {
                ...
                if($privilege_flag == 'Yes')
                {
					...
					if(isset($params_arr['userid']) && $params_arr['userid'] !='')
					{
					    $oncallrequestdata = $oncallsmodel->saveoncallrequest($params_arr['userid'],$params_arr['reason'],$params_arr['oncall_type_id'],$params_arr['oncall_day'],$params_arr['from_date'],$params_arr['to_date'],$params_arr['oncallcount']);
					    $status = $oncallrequestdata['status'];
						$message = $oncallrequestdata['message'];
						$result = $oncallrequestdata['result'];
						//echo "<pre>";print_r($oncallrequestdata);exit;
		...
		public function manageemployeeoncalls($params_arr)
        {
            $result = array();
			$status = 0;
			$message = "No data found.";
            ...
            if(isset($params_arr['role_id']) && $params_arr['role_id'] != '' && isset($params_arr['group_id']) && $params_arr['group_id'] != '')
            {
                ...
                if($privilege_flag == 'Yes')
                {
					...
					{
						$oncallsmodel = new Services_Model_Oncalls();
						$total_data = $oncallsmodel->getEmployeeOncalldata($page_no,$per_page,$params_arr['userid'],$status_flag);
						if(!empty($total_data['rows']) && count($total_data['rows']) > 0)
						{
						    $result = $total_data['rows'];
							$page_cnt = $total_data['page_cnt'];
		...
		public function manageindividualemponcall($params_arr)
        {
            $result = array();
			$status = 0;
			$message = "No data found.";
            ...
            if(isset($params_arr['role_id']) && $params_arr['role_id'] != '' && isset($params_arr['group_id']) && $params_arr['group_id'] != '')
            {
                ...
                if($privilege_flag == 'Yes' && $edit_flag == 'Yes')
                {
					...
					{
						$oncallsmodel = new Services_Model_Oncalls();
						$total_data = $oncallsmodel->getIndividulEmpOncalldata($params_arr['userid'],$params_arr['recordid'],$params_arr['employeeid']);
						if(!empty($total_data['rows']) && count($total_data['rows']) > 0)
						{
						    $result = $total_data['rows'];
						 	$message = "success";
		...
		public function viewindividualoncall($params_arr)
        {
            $result = array();
			$status = 0;
			$message = "No data found.";
            ...
            if(isset($params_arr['role_id']) && $params_arr['role_id'] != '' && isset($params_arr['group_id']) && $params_arr['group_id'] != '')
            {
                ...
                if($privilege_flag == 'Yes')
                {
					if(isset($params_arr['userid']) && $params_arr['userid'] !='' && isset($params_arr['recordid']) && $params_arr['recordid'] !='' && isset($params_arr['oncallflag']) && $params_arr['oncallflag'] !='')
					{
						if(isset($params_arr['oncallflag']))
						{
							if($params_arr['oncallflag'] == 'pending')
						...
						$oncallsmodel = new Services_Model_Oncalls();
						$total_data = $oncallsmodel->getIndividulOncalldata($params_arr['userid'],$params_arr['recordid']);
						//echo "<pre>";print_r($total_data);exit;
						if(!empty($total_data['rows']) && count($total_data['rows']) > 0)
						{
						    $result = $total_data['rows'];
							...
							if($status_flag == 1)
							{
								$cancel_flag = sapp_Global::_checkprivileges(PENDINGONCALLS,$group_id,$role_id,"delete");
							}
						}
					...
					}else
					{
					    if($params_arr['userid'] == '')
						    ...
						    $message = "Record Id cannot be empty.";
						else if($params_arr['oncallflag'] == '')
						    $message = "Oncall flag cannot be empty.";
					}
		...
		public function approveemponcall($params_arr)
        {
            $result = array();
			$status = 0;
			$message = "No data found.";
            ...
            if(isset($params_arr['role_id']) && $params_arr['role_id'] != '' && isset($params_arr['group_id']) && $params_arr['group_id'] != '')
            {
                ...
                if($privilege_flag == 'Yes' && $edit_flag == 'Yes')
                {
					if(isset($params_arr['userid']) && $params_arr['userid'] !='' && isset($params_arr['recordid']) && $params_arr['recordid'] !='' && isset($params_arr['employeeid']) && $params_arr['employeeid'] !='' && isset($params_arr['actionflag']) && $params_arr['actionflag'] !='')
					{
						...
						$oncallsmodel = new Services_Model_Oncalls();
						if($actionstatus == 2 || $actionstatus == 3)
						   $total_data = $oncallsmodel->approveEmpOncalldata($params_arr['userid'],$params_arr['recordid'],$params_arr['employeeid'],$actionstatus);
		...
		public function canceloncall($params_arr)
        {
            $result = array();
			$status = 0;
			$message = "No data found.";
            ...
            if(isset($params_arr['role_id']) && $params_arr['role_id'] != '' && isset($params_arr['group_id']) && $params_arr['group_id'] != '')
            {
                ...
                $privilege_flag = sapp_Global::_check_menu_access(PENDINGONCALLS,$group_id,$role_id);
				$delete_flag = sapp_Global::_checkprivileges(PENDINGONCALLS,$group_id,$role_id,"delete");
                if($privilege_flag == 'Yes' && $delete_flag == 'Yes')
                {
					if(isset($params_arr['userid']) && $params_arr['userid'] !='' && isset($params_arr['recordid']) && $params_arr['recordid'] !='')
					{
						$oncallsmodel = new Services_Model_Oncalls();
						$total_data = $oncallsmodel->canceloncall($params_arr['userid'],$params_arr['recordid']);
