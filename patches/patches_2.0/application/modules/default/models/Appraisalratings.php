...
class Default_Model_Appraisalratings extends Zend_Db_Table_Abstract
{
	...
	public function getAppraisalRatingsData($sort, $by, $pageNo, $perPage,$searchQuery,$implementationflag)
	{
		...
		{
			$where.= " AND i.businessunit_id = $businessUnit ";
			if($implementationflag !='')
			{
				if($implementationflag == 0)
					$where.= " AND i.department_id=$department";
			}
		}
		
		/**
		** query change 10-04-2015
		** done by soujanya
		
		$appraisalRatingsData = $this->select()
    					   ->setIntegrityCheck(false)	
                           ->from(array('r'=>'main_pa_ratings'),array('r.*','rating' =>'concat(r.rating_value,"-",r.rating_text)'))
                           ->joinLeft(array('c'=>'main_pa_implementation'), 'c.id = r.pa_configured_id', array('performance_app_flag'=>'if (c.performance_app_flag=1,"Business Unit wise","Department Wise")','appraisal_ratings'=>'if (c.appraisal_ratings=1,"1-5","1-10")'))
                           ->joinLeft(array('i'=>'main_pa_initialization'), 'i.id = r.pa_initialization_id', array('period'=>'concat(i.appraisal_mode," ",i.appraisal_period,"-",year(i.createddate))','i.status','i.enable_step','i.employee_response','i.initialize_status'))
		...
		**/
		$appraisalRatingsData = $this->select()
    					   ->setIntegrityCheck(false)	
                           ->from(array('r'=>'main_pa_ratings'),array('r.*','rating' =>'concat(r.rating_value,"-",r.rating_text)'))
							->joinInner(array('i'=>'main_pa_initialization'), 'i.id = r.pa_initialization_id', array('period'=>'concat(case when i.appraisal_mode = "Yearly" then "Y" when i.appraisal_mode = "Half-yearly" then "H" when i.appraisal_mode = "Quarterly" then "Q" end,"",i.appraisal_period," Appraisal, ",i.to_year)','i.status','i.enable_step','i.employee_response','i.initialize_status','appraisal_ratings'=>'if (i.appraisal_ratings=1,"1-5","1-10")'))
	...
	{		
		...
		$tableFields = array('action'=>'Action','period'=>'Appraisal Period','unitname'=>'Business Unit','deptname' => 'Department','appraisal_ratings'=>'Rating Type');
